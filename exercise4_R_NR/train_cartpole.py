...
def train_online(env, agent, num_episodes, model_dir="./models_cartpole", tensorboard_dir="./tensorboard",  eval_episodes=5, eval_interval=100):
    if not os.path.exists(model_dir):
        os.mkdir(model_dir)  
 
    print("... train agent")

    tensorboard = Evaluation(os.path.join(tensorboard_dir, "train"), ["episode_reward", "eval_reward","a_0", "a_1"])
    ...
    for i in range(num_episodes):
        ...
        #if i % 20 == 0:
        #    greedy_stats = run_episode(env, agent, deterministic=True, do_training=False,  rendering=False)
        #    print("greedy reward: ",  greedy_stats.episode_reward)
