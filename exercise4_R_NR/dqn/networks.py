...
class CNN(NeuralNetwork):
    """
    Convolutional Network class based on TensorFlow.
    """        
    def __init__(self, state_shape, num_actions, hidden=20, lr=1e-4):
        self._build_model(state_shape, num_actions, hidden, lr)

    def _build_model(self, state_shape, num_actions, hidden, lr):
        """
        This method creates a neural network with two hidden fully connected layers and 20 neurons each. The output layer
        has #a neurons, where #a is the number of actions and has linear activation.
        Also creates its loss (mean squared loss) and its optimizer (e.g. Adam with a learning rate of 1e-4).
        ...
        self.states_ = tf.placeholder(tf.float32, shape=[None]+state_shape)
        self.actions_ = tf.placeholder(tf.int32, shape=[None])                  # Integer id of which action was selected
        self.targets_ = tf.placeholder(tf.float32,  shape=[None])               # The TD target value

        # network
        conv_layer1 = tf.layers.conv2d(self.states_,  16,  3,  padding="same",  activation=tf.nn.relu)
        ...
        self.predictions = tf.layers.dense(dense_layer1,  num_actions)
...
class CNNTargetNetwork(CNN):
    ...
    it is always set to the values of its associate.
    """
    def __init__(self, state_shape, num_actions, hidden=20, lr=1e-4, tau=0.01):
        super(CNNTargetNetwork,  self).__init__(state_shape, num_actions, hidden, lr)
        self.tau = tau
        self._associate = self._register_associate()
